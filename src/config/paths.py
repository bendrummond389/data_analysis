...
def find_nearest_config(
    ...
    raise FileNotFoundError(f"No {config_name} found in hierarchy starting from {start_path}")
...
def create_symlink(source: Path, target: Path):
    ...
    if target.exists():
        print(f"{target} already exists. Delete it or rename it before creating a symlink.")
        return
